...
import com.example.LMDb.adapters.OptionType;
 ...
 */
public class WatchedMoviesFragment extends Fragment implements MoviesAdapter.MovieOptionsListener {
    ...
    private void loadWatchedMovies() {
        ...
        db.collection("users").document(uid).collection("watchHistory")
                ...
                .addOnFailureListener(e -> {
                    Toast.makeText(getContext(), "Failed to load your watch history", Toast.LENGTH_SHORT).show();
                });
    }
    ...
    public void onOptionSelected(Movie movie, MoviesAdapter.OptionType option) {
        if (option == MoviesAdapter.OptionType.REMOVE_FROM_WATCH_HISTORY) {
            removeMovieFromWatchHistory(movie);
        }
    }

    private void removeMovieFromWatchHistory(Movie movie) {
        String uid = FirebaseAuth.getInstance().getUid();
        if (uid == null) return;
